...
# 
# As `ld_mode` is set to 'interp' by default, the `ld_func` parameter is no longer visible.  If setting `ld_func` to something other than 'interp', you must first change the value of `ld_mode` (to either 'func_provided' which allows setting `ld_coeffs` as in previous releases, or to 'func_lookup' which is the new behavior introduced in 2.2).
# 
